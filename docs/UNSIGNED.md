# Running unsigned ATEM Fleet Admin builds
...
ATEM Fleet Admin listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of ATEM Fleet Admin**
...
domain. ATEM Fleet Admin needs this to serve the web UI and push configuration to ATEMs over the network.

If you deny it (or the prompt appears behind another window and times out), "Connect & apply" will fail — the folder-export path still works offline
...
**Private**. If ATEM Fleet Admin isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "ATEM Fleet Admin" -Direction Inbound -Program "C:\Path\To\ATEM Fleet Admin.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*ATEM Fleet Admin*" | Remove-NetFirewallRule
```
...
```sh
codesign --force --deep --sign - "/Applications/ATEM Fleet Admin.app"
```

Verify it took:

```sh
codesign -dv --verbose=4 "/Applications/ATEM Fleet Admin.app"
spctl -a -vv "/Applications/ATEM Fleet Admin.app"   # still reports "rejected" — ad-hoc is not notarization
```
