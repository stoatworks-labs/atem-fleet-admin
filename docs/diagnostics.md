...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/atem-fleet-admin/` |
| Linux | `$XDG_STATE_HOME/atem-fleet-admin/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\atem-fleet-admin\logs\` |

`ATEM_FLEET_ADMIN_LOG_DIR` overrides it. The path is printed on the first line of every
...
`atem-fleet-admin.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `ATEM_FLEET_ADMIN_LOG`, default `info`.
...
```bash
atem-fleet-admin --collect-diagnostics
```

Writes `atem-fleet-admin-diagnostics-<timestamp>.json` and prints its path. One file, so
