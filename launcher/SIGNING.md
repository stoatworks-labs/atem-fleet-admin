# Signing & notarizing the ATEM Fleet Admin desktop app
...
The Electron desktop build from `release.yml` is handled the same way: its
macOS images are published ad-hoc signed and notarised post-hoc by the same
watcher.
...
npm ci
bash scripts/prepare.sh          # build the web app + server and embed the Node runtime (NODE_PLATFORM=darwin-universal for both architectures)
